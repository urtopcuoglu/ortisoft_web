...
export default async function AdminPagesPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-extrabold text-slate-900">Sayfa Yönetimi</h1>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {pages.map((page) => (
              <tr key={page.key} className="border-b border-slate-100 last:border-0">
                <td className="px-4 py-3 font-semibold text-slate-800">{page.label}</td>
                <td className="px-4 py-3 font-mono text-xs text-slate-500">{PAGE_PATH[page.key] ?? "—"}</td>
